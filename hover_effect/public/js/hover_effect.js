...
$(document).ready(function () {
  ...
  function get_hover_target(element) {
    const $el = $(element);
    if ($el.closest("#custom-hover-popover").length) return null;

    const attr_target = get_attribute_target($el);
    if (attr_target) return attr_target;

    const list_target = get_list_target($el);
    if (list_target) return list_target;
  ...
  function get_attribute_target($el) {
    ...
    return {
      doctype: frappe.utils?.unescape_html ? frappe.utils.unescape_html(doctype) : doctype,
      name: frappe.utils?.unescape_html ? frappe.utils.unescape_html(name) : name,
    };
  }

  function get_list_target($el) {
    if (!window.cur_list?.doctype) return null;
    ...
    if (!name) {
      ...
      name = window.cur_list?.data?.[idx]?.name;
    }
  ...
  function get_href_target($el) {
    ...
    if (parts[0] !== "app" || parts.length < 3) return null;
  ...
  function resolve_doctype_from_slug(slug) {
    if (!slug || !frappe.router?.slug) return null;

    if (!slug_map) {
      slug_map = {};
      const doctypes = new Set([window.cur_list?.doctype]);

      (frappe.boot?.link_preview_doctypes || []).forEach((doctype) => doctypes.add(doctype));
      Object.keys(frappe.boot?.user?.can_read || {}).forEach((doctype) => doctypes.add(doctype));
      (hover_config?.allowed_doctypes || []).forEach((doctype) => doctypes.add(doctype));
  ...
  async function maybe_show_hover_popover(target) {
    if (!target.doctype && target.doctype_slug) {
      ...
      target.doctype =
        resolve_doctype_from_slug(target.doctype_slug) ||
        resolve_allowed_doctype_from_slug(target.doctype_slug, config);
    }
    ...
    if (current_hovered === hover_key && !$hover_popover.hasClass("hidden")) return;
  ...
  async function is_hover_allowed(doctype) {
    const config = await get_hover_config();
    return Boolean(config?.enabled && (config.allowed_doctypes || []).includes(doctype));
  }

  async function get_hover_config() {
    if (hover_config) return hover_config;

    if (frappe.boot?.hover_effect) {
    ...
    if (!hover_config_promise) {
      hover_config_promise = new Promise((resolve) => {
        frappe.call({
          ...
          callback: function (r) {
            hover_config = r?.message || { enabled: false, allowed_doctypes: [] };
            slug_map = null;
  ...
  function resolve_allowed_doctype_from_slug(slug, config) {
    if (!slug || !config?.allowed_doctypes?.length || !frappe.router?.slug) return null;
  ...
  function has_stored_ai_layout(doctype) {
    return Boolean((hover_config?.stored_ai_layout_doctypes || []).includes(doctype));
  }
  ...
  async function enhance_page_tables() {
    const config = await get_hover_config();
    if (!config?.enabled || !(config.allowed_doctypes || []).length) return;

    $(".page-container table td, .layout-main table td, .frappe-card table td")
      .not(".hover-effect-processed")
      .each(function () {
        const $cell = $(this);
        $cell.addClass("hover-effect-processed");

        if ($cell.find("a, button, input, .ma-pill, .status-pill").length) return;

        const text = normalize_cell_text($cell.text());
        if (!is_link_candidate(text)) return;
  ...
  function convert_cell_to_link($cell, target, label) {
    if (!target?.doctype || !target?.name || !frappe.router?.slug) return;
  ...
  function resolve_currency_code(optionValue, doc, fallbackCurrency) {
    if (!optionValue) return fallbackCurrency;
    if (!String(optionValue).includes(":")) {
      return doc?.[optionValue] || optionValue || fallbackCurrency;
    }

    const parts = String(optionValue).split(":");
    const fieldname = parts[parts.length - 1];
    return doc?.[fieldname] || fallbackCurrency;
  }
  ...
  function format_generic_value(field, fallbackCurrency, doc = null) {
    ...
    const currency = resolve_currency_code(field?.options, doc, fallbackCurrency);

    if (value === undefined || value === null || value === "") return "—";

    if (["Date", "Datetime"].includes(fieldtype)) return esc(fmt_date(value));
    if (fieldtype === "Currency") return esc(fmt_currency(value, currency));
    if (["Float", "Percent"].includes(fieldtype)) return esc(fmt_number(value));
    ...
    if (["Link", "Dynamic Link"].includes(fieldtype) && value) {
      const linkedDoctype =
        fieldtype === "Link" ? field.options : doc_field_link_doctype(field, doc);
      if (linkedDoctype && frappe.router?.slug) {
  ...
  function render_header(doctype, name, doc = {}, options = {}) {
    const slug = frappe.router?.slug ? frappe.router.slug(doctype) : "";
    ...
    const geminiEnabled = Boolean(hover_config?.gemini_enabled);
    const aiButtonLabel = options.aiPending
      ? "Generating Layout..."
      : options.hasStoredAiLayout
        ? "Generate New Layout"
        : "Generate New Layout";
    ...
    return `
            <div class="hover-header">
                <div class="hover-header-copy">
                    ...
                    <div class="hover-subtitle">Owner ${owner} · Updated ${lastUpdated}</div>
                </div>
                <div class="hover-header-actions">
                    ${
                      geminiEnabled
                        ? `${
                            options.hasStoredAiLayout
                              ? `<button class="hover-ai-secondary-button">Default View</button>`
                              : ""
                          }<button class="hover-ai-button" ${
                            options.aiPending ? "disabled" : ""
  ...
  function render_linked_sections(sections, fallbackCurrency) {
    ...
    const cardsHtml = sections
      .map((section) => {
        const slug = frappe.router?.slug ? frappe.router.slug(section.doctype) : "";
        const href = slug ? `/app/${slug}/${encodeURIComponent(section.name)}` : "#";
        const rowsHtml = (section.rows || [])
          .map(
            (row) => `<div class="hover-meta-item">
                <span class="hover-meta-label">${esc(row.label)}</span>
                <span class="hover-meta-value ${
                  row.bold ? "hover-bold" : ""
                }">${format_generic_value(row, fallbackCurrency, row.__doc)}</span>
            </div>`
          )
          .join("");
  ...
  function render_popover_content(data, doctype, name) {
    ...
    const status = status_row?.value || doc.status || (doc.disabled ? "Disabled" : "");
    const status_cls = status ? get_status_class(status) : "";
    const meta_rows = (data.meta_rows || [])
      .filter((row) => row.fieldname !== "status")
      .map((row) => ({
        label: row.label,
    ...
    const meta_html = `
            <div class="hover-meta-grid">
                ...
                ${meta_rows
                  .map(
                    (row) =>
                      `<div class="hover-meta-item"><span class="hover-meta-label">${esc(
                        row.label
                      )}</span><span class="hover-meta-value ${row.bold ? "hover-bold" : ""}">${
                        row.value
                      }</span></div>`
                  )
                  .join("")}
            </div>`;
    const detailSections = (data.detail_sections || []).map((section) => ({
      ...section,
      rows: (section.rows || []).map((row) => ({ ...row, __doc: doc })),
    }));
    const linkedSections = (data.linked_sections || []).map((section) => ({
      ...section,
      rows: (section.rows || []).map((row) => ({ ...row, __doc: section.doc || null })),
    }));
    ...
    const useSingleColumn =
      (!hasLinkedSections && !hasTableSections && !hasRelatedSections) ||
      detailRowCount > 18 ||
      detailSections.length > 4;
    const layoutClass = useSingleColumn ? "hover-layout single-column" : "hover-layout";
    const secondaryColumnHtml =
      hasLinkedSections || hasTableSections || hasRelatedSections
        ...
        : "";
    const detailBlockHtml = hasDetailSections
      ? `<div class="hover-card">${detail_sections_html}</div>`
      : `<div class="hover-card"><div class="hover-empty-note">No additional fields to show for this record.</div></div>`;
  ...
  function render_ai_popover_content(aiLayout, data, doctype, name) {
    ...
    const status = statusRow?.value || doc.status || (doc.disabled ? "Disabled" : "");
    const detailSections = (data.detail_sections || []).map((section) => ({
      ...section,
      rows: (section.rows || []).map((row) => ({ ...row, __doc: doc })),
    }));
    ...
    const summaryHtml = (data.meta_rows || [])
      .filter((row) => row.fieldname !== "status")
      .slice(0, 8)
      .map(
      ...
      .join("");
    const fieldSectionsHtml = detailSections
      .map((section) => {
        const rowsHtml = (section.rows || [])
          .map(
      ...
      .join("");
    const linkedSectionsHtml = linkedSections.length
      ? `<section class="hover-ai-section">
      ...
      : "";
    const tableSectionsHtml = [...(data.table_sections || []), ...(data.related_sections || [])]
      .map((section) => {
                ...
                <div class="hover-ai-shell">
                    <div class="hover-ai-root">
                        <section class="hover-ai-banner">
                            ...
                            ${
                              status
                                ? `<div class="hover-ai-status"><span class="status-pill ${get_status_class(
                                    status
                                  )}">${esc(status)}</span></div>`
                                : ""
                            }
                    ...
                    ${
                      aiLayout.summary
                        ? `<div class="hover-ai-note">${esc(aiLayout.summary)}</div>`
                        : `<div class="hover-ai-note">Gemini formatted this popup using the current record details.</div>`
                    }
                </div>
            </div>`;
  ...
  function load_ai_layout(regenerate = true) {
    if (!current_target || !current_preview) return;
    ...
    frappe.call({
      ...
      callback: function (r) {
        ...
        if (!r?.message?.css) {
          frappe.show_alert({
            message: r?.message?.error || __("Gemini could not format this popup."),
            indicator: "orange",
          });
        ...
        if (!(hover_config?.stored_ai_layout_doctypes || []).includes(current_target.doctype)) {
      ...
      error: function (xhr) {
        ...
        const message =
          xhr?.message ||
          xhr?._server_messages ||
          __("Gemini formatting failed. Check API key or model configuration.");
  ...
  function clear_ai_layout() {
    ...
    frappe.call({
      ...
      error: function (xhr) {
        const message =
          xhr?.message || xhr?._server_messages || __("Could not clear the stored Gemini layout.");
        frappe.show_alert({ message, indicator: "red" });
      },
    });
  }
