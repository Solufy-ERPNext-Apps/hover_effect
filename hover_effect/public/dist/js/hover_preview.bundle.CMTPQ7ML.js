(() => {
  // ../hover_effect/hover_effect/public/js/hover_preview.bundle.js
  ...
  $(document).ready(function() {
    ...
    $(document).on("mouseenter", ".list-row-container", function(e) {
      ...
      if (!name) {
        let $id_link = $(this).find(".list-row-col .list-subject a, .list-row-col:nth-child(4) span, a.name");
        if ($id_link.length) {
          name = $id_link.text().trim();
        }
      }
      if (!name) {
        console.log("Could not find data-name on hover");
        return;
      }
      console.log("Hovering over Document ID: ", name);
      clearTimeout(hover_timeout);
      hover_timeout = setTimeout(() => {
        current_hovered = name;
        show_hover_popover(cur_list.doctype, cur_list.data.name, e.clientX, e.clientY);
      }, 500);
    });
    ...
    $(document).on("mouseleave", ".list-row", function(e) {
      clearTimeout(hover_timeout);
      setTimeout(() => {
        if (!$hover_popover.is(":hover")) {
          hide_hover_popover();
...
//# sourceMappingURL=hover_preview.bundle.CMTPQ7ML.js.map
